import { SavedCardWrapper } from "../../../pageStyles/styles";
import { SavedCard } from "@lk/ui-library";
...
const SavedCards = ({
  ...
  const savedCardList = useSelector((state: RootState) => state.saveCardInfo);
  const { isDeletedSavedCardSuccess } = useSelector(
    (state: RootState) => state.saveCardInfo
  );
  const userInfo = useSelector((state: RootState) => state.userInfo);
  useEffect(() => {
    if (userData.id && userData.isLoggedIn && !isDeletedSavedCardSuccess) {
      dispatch(fetchUserDetails({ sessionId: userData.id }));
      dispatch(savedCardData({ sessionId: userData.id, query: "" }));
    }
    if (isDeletedSavedCardSuccess) {
      dispatch(savedCardData({ sessionId: userData.id, query: "" }));
    }
  }, [userData, isDeletedSavedCardSuccess]);

  const onDeleteCard = (params: any) => {
    const obj = {
      cardToken: params,
      email: userInfo.email,
    };
    dispatch(deleteSavedCardData({ ...obj, sessionId: userData.id }));
  };
  useCustomerState({
    useMounted: false,
    userData: userData,
    ...
    >
      <SavedCardWrapper>
        <SavedCard
          ...
          savedCardList={savedCardList.data}
